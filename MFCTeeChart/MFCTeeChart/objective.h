...
void objectives(vector<double> x_var, vector <double> &y_obj)
{
	...
	if (!strcmp(strTestInstance, "DTLZ1"))
	{
		if (numObjectives == 3) {
			...
			for (int n = 1; n < numVariables; n++)
				g = g + pow(x_var[n] - 0.5, 2) - cos(20 * pi*(x_var[n] - 0.5));
			g = 100 * (numVariables - 1 + g);
			y_obj[0] = 0.5*(1 + g)*x_var[0];
			y_obj[1] = 0.5*(1 + g)* (1 - x_var[0]);
	...
	if (!strcmp(strTestInstance, "DTLZ2"))
	{
		if (numObjectives == 3) {
			...
			y_obj[0] = (1 + g)*cos(x_var[0] * pi / 2);
			y_obj[1] = (1 + g)*sin(x_var[0] * pi / 2);
	...
	if (!strcmp(strTestInstance, "DTLZ4"))
	{
		if (numObjectives == 3) {
			...
			y_obj[0] = (1 + g)*cos(pow(x_var[0], pi) * pi / 2)*cos(pow(x_var[1], pi) * pi / 2);
			y_obj[1] = (1 + g)*cos(pow(x_var[0], pi) * pi / 2)*sin(pow(x_var[1], pi) * pi / 2);

			y_obj[2] = (1 + g)*sin(pow(x_var[0], pi) * pi / 2);
		}
		else {
			double g = 0;
			double xx = (x_var[0] + x_var[1]) / 2.0;
			...
			y_obj[0] = (1 + g)*cos(pow(x_var[0], pi) * pi / 2);
			y_obj[1] = (1 + g)*sin(pow(x_var[0], pi) * pi / 2);
