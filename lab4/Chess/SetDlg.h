...
public:
	int GetSelectedEngine(){return m_nSelectedEngine;};//取用户选中的搜索引擎号码
	int GetSelectedPly(){return m_nSelectedPly;};      //取用户选中的搜索深度
	int GetUserChessColor(){return m_nUserChessColor;};//取用户选中的棋子颜色
	int GetChessSort(){return m_nChessSort;};          //取下棋方式
	int GetDepthSort(){return m_iDepthSort;};          //取选择深度方式
...
protected:
	...
	afx_msg void OnRadioDefaultset();
